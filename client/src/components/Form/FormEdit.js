...
export default function FormEdit(props) {
  ...
  useEffect(() => {
    async function fetchUserData() {
      console.log(id);
      try {
        const resp = await fetch(`${apiUrl}/players/${id}`);
        const data = await resp.json();
        dispatchUsername({ type: "USER_INPUT", val: data.message.username });
        dispatchPassword({ type: "USER_INPUT", val: data.message.password });
        dispatchEmail({ type: "USER_INPUT", val: data.message.email });
        dispatchExp({ type: "USER_INPUT", val: data.message.exp });
